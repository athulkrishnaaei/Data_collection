#!/usr/bin/env python3
# ───------------------- HOT-FIX for weights saved on Python ≥ 3.13 -------------------
import sys, types, pathlib
if sys.version_info < (3, 13):
    stub = types.ModuleType('pathlib._local')
    # expose all concrete and pure Path classes expected in the pickle
    for cls_name in (
        'Path', 'PosixPath', 'WindowsPath', 'PurePath',
        'PurePosixPath', 'PureWindowsPath'
    ):
        setattr(stub, cls_name, getattr(pathlib, cls_name))
    # register the stub so import machinery can find it
...
try:
    while True:
        ...
        for *xyxy, conf, cls_i in dets.cpu().numpy():
            ...
            if not args.headless:
                col = (0, 0, 255) if 'rotten' in lbl.lower() else (0, 255, 0)
                x1f, y1f, x2f, y2f = x1, y1_ + y0, x2, y2_ + y0
                cv2.rectangle(display_frame, (x1f, y1f), (x2f, y2f), col, 2)
                cv2.putText(display_frame, f'{lbl} {conf:.2f}', (x1f, y1f - 6),
            ...
            if TAKE_PHOTO_ENABLED:
                ...
                if not args.quiet:
                    print(f"[INFO] Saved {fname} to {out_dir}")
